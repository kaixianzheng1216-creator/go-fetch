...
func TestBuildEventInput(testRunner *testing.T) {
	...
	if actual.EventType != domain.EventTypeCustom {
		testRunner.Fatalf("EventType = %d, want %d", actual.EventType, domain.EventTypeCustom)
	}
...
func TestBuildEventInputDefaultsPageview(testRunner *testing.T) {
	...
	actual := buildEventInput(request, domain.CollectPayload{
		WebsiteID: "8a7e7a10-7b51-43ef-9e85-874df7dd5f8b",
		URL:       "not a valid url",
	}, now)

	if actual.EventType != domain.EventTypePageView {
		testRunner.Fatalf("EventType = %d, want %d", actual.EventType, domain.EventTypePageView)
	}
