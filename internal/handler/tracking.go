...
type collectRequest struct {
	Body requestdto.CollectRequest
}

func (handler CollectHandler) CollectEvent(ctx context.Context, input *collectRequest) (*responsedto.OKOutput, error) {
	err := handler.collect.Collect(ctx, handler.requestFromContext(ctx), string(input.Body.Type), requestdto.ToCollectPayload(input.Body.Payload))
	if err != nil {
		switch {
